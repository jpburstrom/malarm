...
class ParamBox(QtGui.QFrame):
    ...
    def __init__(self, *args):
        ...
        self._standardActions = {} #StandardAction=>(widget, widget atom, atom type)
        ...
        self.connect(QtGui.qApp, QtCore.SIGNAL("standardParamActivated"), self._handleStandardAction)
    ...
    def _handleStandardAction(self, x):
        pos = self.mapFromGlobal(self._cursor.pos())
        if not self.rect().contains(pos):
            return
        action = self._standardActions[x]
        p = StandardActionButton(action[0], self)
        p.setText(x)
        p.show()
    ...
    def _collectParams(self, force = False):
        ...
        for child in self.findChildren(QtGui.QWidget):
            try:
                ...
                try:
                    x, y = child.standardAction.x(), child.standardAction.y()
                except AttributeError:
                    x, y = child.standardAction, -1
                for z in x, y:
                    if z < 0:
                        continue
                    try:
                        self._standardActions[standardactions.ACTIONS[z][0]].append((child.param, 1, standardactions.ACTIONS[z][1]))
                    except KeyError:
                        self._standardActions[standardactions.ACTIONS[z][0]] = [(child.param, 1, standardactions.ACTIONS[z][1])]
        self._params = p
    ...
    def setSelected(self, boo):
        self._selected = boo
        #XXX Ett snyggare sätt att markera aktuell box?
        if self._selected:
            self.setLineWidth(2)
        else:
            self.setLineWidth(0)
